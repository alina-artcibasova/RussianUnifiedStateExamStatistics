...
$(document).ready(function(){$('body').prepend('<style>.math_ed_butt {background:white; border:1px solid black; margin:2px 1px; padding:2px 3px;cursor:pointer; border-radius:2px; display:inline-block; height:18px;vertical-align:middle; line-height:18px} .math_ed_butt span{} .meb_big {height:30px;line-height:30px}</style>');});
    mbuttons = [
    ['<img src="/formula/03/033b571c237d78ae1c9908427fdf52ce.png" class="tex" style="vertical-align:-11px" width="9">','\\frac{a}{b}'],
    ...
    ['отв', '<input class="test_inp" name="answer_part_1" size=5>'],
...
prepare_editor = function(){
    ...
    $('.math_editor').each(function(index,elem){
        ...
        special={};
        special[1]=$('<div class="math_ed_butt">Формула</div>').data('ta', ta).click(function(){
                ...
                div.append('<br><br>').append(span).append('<br><br>').append($('<span style="border:1px solid black; margin:2px; cursor:pointer; padding:2px">OK</span>').click(function(){insertAtCaret(textarea[0], '<math>'+span.mathquill('latex')+'</math>'); div.remove(); jq_preview(textarea.attr('id').replace('editor_', ''), true)}));
                $('body').append(div)
            });
        special[3] = $('<div class="math_ed_butt">Соотв.</div>').data('ta', ta).click(function(){
            ...
            jq_preview($(this).data('ta').attr('id').replace('editor_', ''), true)
        });
        ...
        special[5] = $('<div class="math_ed_butt">Вар. отв</div>').data('ta', ta).click(function(){
            var cols = prompt('Количество столбцов', 1);
            var rows = prompt('Количество строк', 1);
            var html = '';
            var num = 0;
            for (var j=0; j<rows; j++){
                html += '<tr>';
                for (var i=0; i<cols; i++){
                    num++;
                    html += '<td style="border:0;width:'+(600/parseInt(cols))+'px">'+(num)+')</td>';
                }
                html+='</tr>';
            }
            insertAtCaret($(this).data('ta')[0], '\n\n<table style="width:600px;margin:auto; border:0">'+html+'</table>\n\n');
	...
	special[2] = $('<div class="math_ed_butt">Табл.</div>').data('ta', ta).click(function(){
	
	var t = this;
    var tds = {};
    var table = $('<table border=1>');
    var main_div = $('<div style="position:fixed; top:50px; left:100px; width:'+($(window).width()-200)+'px; z-index:100; background:white; border:1px solid black; border-radius:5px; overflow:auto;padding:5px; height:'+($(window).height()-100)+'px">');
    ...
    main_div.append('Столбцов: ', $('<input value=0 id="tc_inp_cols" size=2>').change(redraw_table), ' ' + 
    ...
    var insert_table = function(){
        ...
        html += '</table>\n\n';
            ...
            jq_preview($(t).data('ta').attr('id').replace('editor_', ''), true);
        main_div.remove();    

};
    main_div.append(' ', $('<input type="button" value="Вставить">').click(insert_table), ' ', $('<input type="button" value="Закрыть">').click(function(){main_div.remove()}) )
    ...
    $('body').append(main_div);
        ...
        for (inum in gbuttons){
           var item = gbuttons[inum];
           if (item.length == 0){
            ta.before('<br>');
            continue
            }
           if (item.length == 1){
            ta.before(special[item[0]]);
            continue
            }
           ta.before($('<div class="math_ed_butt">').html('<span>'+item[0]+'</span>').data('tpl', item[1]).data('ta', elem).click(function(){
